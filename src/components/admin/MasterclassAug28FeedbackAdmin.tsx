...
const KIND = "mc_aug28_feedback";
const SURVEY_PATH = "/mc-aug28-feedback";
...
const SECTION_RATINGS: { key: keyof FeedbackResponse; label: string }[] = [
  ...
  { key: "rating_presentations", label: "Präsentationen" },
  { key: "rating_future", label: "Zukunft" },
...
const MasterclassAug28FeedbackAdmin = () => {
  ...
  const generateLink = async () => {
    ...
    if (data?.token) {
      setFeedbackLink(`${window.location.origin}${SURVEY_PATH}?token=${data.token}`);
      await loadData();
    }
    setGenerating(false);
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            <Link2 className="w-5 h-5" /> 28.08.26 Masterclass Feedback Link
          </CardTitle>
          {!feedbackLink && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Badge variant="outline">{npsResponses} Antw.</Badge>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold font-display">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Ø {avgNps !== null ? avgNps.toFixed(1) : "—"} · {promoters} Promoters / {detractors} Detractors
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Durchschnitt pro Abschnitt</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Feedback-Antworten</CardTitle>
          <Button
            variant="outline"
            size="sm"
            disabled={!responses.length}
            onClick={() => exportToXlsx(responses, "mc-aug28-feedback", "28.08.26 Masterclass Feedback")}
          >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Teilnehmer:in</TableHead>
                  <TableHead>Gesamt</TableHead>
                  <TableHead>NPS</TableHead>
                  <TableHead>Empfehlen</TableHead>
                      ...
                      <TableRow key={`${r.id}-detail`}>
                        <TableCell colSpan={7}>
                          <div className="grid gap-3 p-4 bg-muted/50 rounded-lg text-sm">
                            ...
                            <div><strong>Gesamt:</strong> {r.overall_rating ?? "—"} / 5 · <strong>NPS:</strong> {r.nps_score ?? "—"}</div>
                            ...
                            {r.best_part && <div><strong>Highlight:</strong> {r.best_part}</div>}
                            {r.improve_part && <div><strong>Zu verbessern:</strong> {r.improve_part}</div>}
                            {r.app_built_description && <div><strong>Gebaut:</strong> {r.app_built_description}</div>}
                            <div>
                              <strong>Baut weiter:</strong> {r.will_continue_building || "—"} · <strong>Empfehlen:</strong> {r.recommend_to_others || "—"}
                            </div>
                            {r.testimonial && (
                              <div>
                                <strong>Testimonial:</strong> {r.testimonial}
                                {r.allow_testimonial_public && (
                                  <Badge variant="default" className="ml-2">Öffentlich OK</Badge>
                                )}
                              </div>
                            )}
                            {r.anything_else && <div><strong>Sonstiges:</strong> {r.anything_else}</div>}
                          </div>
                        </TableCell>
                      </TableRow>
                    )}
...
export default MasterclassAug28FeedbackAdmin;