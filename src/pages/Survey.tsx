...
const BUILDING_BLOCK_CHIPS = ["Email", "Payments (e.g. Stripe)", "User login", "CRM (e.g. HubSpot)", "Accounting (e.g. Bexio)", "Database / storage", "File uploads", "Maps / location", "Calendar", "API integrations", "AI features (e.g. OpenAI)"];
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <div className="relative flex-1">
                                <Textarea {...field} placeholder="Even a rough idea is great — we'll help you shape it!" rows={3} className="pr-16" />
                                <div className="absolute right-2 top-2 flex items-center gap-1">
                                  <MicrophoneButton onTranscript={(t) => field.onChange(field.value ? field.value + " " + t : t)} />
                                  {field.value && (
                                    <button type="button" tabIndex={-1} onClick={() => field.onChange("")} className="text-muted-foreground hover:text-foreground transition-colors">
                                      <X className="w-4 h-4" />
                                    </button>
                                  )}
                                </div>
