...
const AI_EXPERIENCE_OPTIONS = ["Noch nie ausprobiert", "Ein bisschen herumgespielt, nichts Ernsthaftes", "Schon eine App gebaut und veröffentlicht"];
const LOVABLE_EXPERIENCE_OPTIONS = ["Davon gehört", "Eine Demo gesehen", "Ein wenig damit experimentiert", "Etwas Echtes damit gebaut"];
const GOAL_CHIPS = ["Meine erste App bauen", "Eine Idee schnell prototypisieren", "Verstehen, was möglich ist", "Mit anderen Builders vernetzen", "Spass haben 🎉"];
const SUCCESS_CHIPS = ["Mit einer funktionierenden App nach Hause gehen", "Lovable selbständig nutzen können", "Klare Roadmap für mein Projekt", "Neue Kontakte geknüpft", "Neues Mindset zum Coden"];
const BUILDING_BLOCK_CHIPS = ["E-Mail", "Zahlungen (z.B. Stripe)", "Benutzer-Login", "CRM (z.B. HubSpot)", "Buchhaltung (z.B. Bexio)", "Datenbank / Speicher", "Datei-Uploads", "Karten / Standort", "Kalender", "API-Integrationen", "KI-Funktionen (z.B. OpenAI)"];

const MasterclassAug28Survey = () => {
  ...
  useEffect(() => {
    ...
    supabase.rpc("validate_survey_token", { _token: token })
      .then(({ data }) => {
        ...
        if (row && row.kind === "mc_aug28_prep") { setTokenId(row.id); setPageState("form"); }
  ...
  const onSubmit = async (values: SurveyFormValues) => {
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        toast({ title: "Bereits abgesendet", description: "Du hast bereits eine Antwort eingereicht. Vielen Dank!", variant: "default" });
        ...
        <Card className="w-full max-w-md text-center">
          <CardHeader>
            ...
            <p className="text-muted-foreground mt-2">
              Vielen Dank für deine Zeit — wir nutzen deine Antworten, um den Workshop für dich grossartig zu machen. Bis bald!
            </p>
          </CardHeader>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h2 className="font-display text-xl font-semibold">Bereit machen für den Workshop! 🚀</h2>
          <p className="text-muted-foreground mt-2">
            Kurze Vorbereitungs-Umfrage — dauert etwa 3 Minuten. Antworten antippen oder selbst eintippen.
          </p>
        </div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-16">
                ...
                <FormField
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <FormItem>
                        <FormLabel className="text-base">🤖 Hast du vor diesem Workshop schon eine Website mit KI-Tools gebaut?</FormLabel>
                        <div className="space-y-3">
                          ...
                          <FormControl>
                            <ClearableInput
                              ...
                              placeholder="Optional: Details hinzufügen…"
                              className="text-sm"
                            />
                          </FormControl>
                        </div>
                ...
                <FormField
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <FormItem>
                        <FormLabel className="text-base">💜 Was sind deine Erfahrungen mit Lovable.dev?</FormLabel>
                        <div className="space-y-3">
                          <ChipSelect
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      <FormLabel className="text-base">🎯 Was möchtest du aus diesem Workshop mitnehmen?</FormLabel>
                      <p className="text-sm text-muted-foreground">Wähle so viele wie du möchtest, oder ergänze deine eigenen</p>
                      <div className="space-y-3">
                        ...
                        <FormControl>
                          <ClearableInput
                            ...
                            placeholder="Etwas anderes hinzufügen…"
                            className="text-sm"
                          />
                        </FormControl>
                      </div>
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      <FormLabel className="text-base">🏆 Wann wäre dieser Tag für dich ein voller Erfolg?</FormLabel>
                      <p className="text-sm text-muted-foreground">Und was wäre eher enttäuschend?</p>
                      <div className="space-y-3">
                        ...
                        <FormItem>
                          ...
                          <div className="flex items-start gap-1">
                            <div className="relative flex-1">
                              <Textarea {...field} placeholder="Auch eine grobe Idee reicht — wir helfen dir, sie zu schärfen!" rows={3} className="pr-8" />
                              {field.value && (
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <ClearableInput
                            placeholder="Bereiche, in denen du Potenzial siehst, etwas zu bauen oder zu automatisieren?"
                            className="text-sm"
                            value={field.value || ""}
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      <FormLabel className="text-base">🔗 Möchtest du deine Web-App mit anderen Systemen integrieren?</FormLabel>
                      <p className="text-sm text-muted-foreground">Tippe alles an, was zutrifft — hilft uns, Beispiele vorzubereiten</p>
                      <div className="space-y-3">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                          <div className="flex items-start gap-1">
                            <div className="relative flex-1">
                              <Textarea {...field} placeholder="Hier kannst du alles weitere ergänzen..." rows={2} className="pr-8" />
                              {field.value && (
                                <button type="button" tabIndex={-1} onClick={() => field.onChange("")} className="absolute right-2 top-2 text-muted-foreground hover:text-foreground transition-colors">
                                  <X className="w-4 h-4" />
                                </button>
...
export default MasterclassAug28Survey;